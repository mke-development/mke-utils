...
import team.mke.utils.ktor.options.OptionsRoutingContext.Companion.sets
import team.mke.utils.ktor.swagger.OpenApiRouteBlock
import team.mke.utils.ktor.swagger.OptionTag
...
class OptionsPluginConfiguration {
    var path = "/options"
    var json: Json = team.mke.utils.ktor.json
...
fun Route.configureOptions(configuration: OptionsPluginConfiguration.() -> Unit) {
    ...
    route(config.path) {
        fun RoutingContext.errorDTO(key: String, method: String) = ErrorDTO(
                ...
                JsonObject(
                    keys.map { key ->
                        OptionHandler.registered[key]?.let { handler ->
                            ...
                            call.respond(HttpStatusCode.NotFound, errorDTO(key, "GET"))
                            return@get
                        }
                    }.toMap()
                )
        ...
        put(config.putDocs) {
            ...
            val options = config.json.decodeFromString<JsonObject>(text).map { (key, value) ->
                ...
                OptionHandler.registered[key]?.let { handler ->
                    ...
                    call.respond(HttpStatusCode.NotFound, errorDTO(key, "PUT"))
                    return@put
                }
            }.toMap()
